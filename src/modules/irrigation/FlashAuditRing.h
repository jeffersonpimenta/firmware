...
// Integridade por-registro NÃO é checada (só CRC do header, contra header rasgado/flash em branco).
class FlashAuditRing {
  ...
  private:
    ...
    uint32_t num = 0;
    void writeHeader();
    size_t slotOffset(uint32_t idx) const {
        size_t cap = capacity();
        return (cap > 0) ? HEADER + (idx % cap) * REC : HEADER;
    }
};
