...
static void test_migrate_v1_preservesFieldsAndDefaultsNew()
{
    ...
    TEST_ASSERT_EQUAL_UINT16(8, s.version);
...
{
    TEST_ASSERT_EQUAL_size_t(208, sizeof(IrrigationSettings));
    IrrigationSettings s;
...
{
    TEST_ASSERT_EQUAL_UINT32(208, sizeof(IrrigationSettings));
...
// Fase 9 (v6→v8): defaults, tamanho e offsets dos limiares de bateria (v6 preservados).
...
// v7 (Modo Remoto): struct v7 tinha 184 bytes; v8 tem 208 bytes.
static void test_settings_v7_size()
{
    TEST_ASSERT_EQUAL_UINT(208, sizeof(IrrigationSettings));
}

// v8 (P2P fallback): blob v7 (184 B) migra para v8 com defaults dos campos novos.
static void test_migrate_v7_to_v8_preserves_prefix_and_defaults()
{
    IrrigationSettings src; // struct atual = v8
    src.version = 7;
...
// v8 (P2P fallback): roundtrip exato de blob v8 (208 B).
static void test_migrate_v8_exact_roundtrip()
{
...
void setup()
{
    ...
    RUN_TEST(test_migrate_v7_to_v8_preserves_prefix_and_defaults);
    RUN_TEST(test_migrate_v8_exact_roundtrip);
    exit(UNITY_END());
}
